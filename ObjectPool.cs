...
public class ObjectPool : MonoBehaviour
{
    ...
    public bool alwaysDestrol = false; // Luôn hủy đối tượng khi không sử dụng
    ...
    private void OnDestroy()
    {
        foreach (var item in objectPool)
        {
            ...
            else if (item.activeSelf == false || alwaysDestrol)
            {
                Destroy(item);
            }
            else
            {
                item.GetComponent<DestroyIfDisabled>().SelDestructionEnabled = true;
            }
        }
    }
}