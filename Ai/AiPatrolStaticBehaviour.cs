...
public class AiPatrolStaticBehaviour : AIBehaviour
{
    ...
    {
        float angle = Vector2.Angle(tank.aimTuret.transform.right, randomDirection);
        if (currentPatrolDelay <= 0 && angle < 10)
        {
            randomDirection = Random.insideUnitCircle;
        ...
        else
        {
            ...
            {
                tank.HandleTurretMovement((Vector2)tank.aimTuret.transform.position + randomDirection);
            }
        }
    }
}
